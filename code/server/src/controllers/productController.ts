...
class ProductController {
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> { 
        ...
        return new Promise<number>((resolve, reject) => {
            (async () => {
                if(!changeDate) changeDate = today;
                else if (changeDate && changeDate > today) {reject(new DateError()); return;}
                try {
                  const product = (await this.dao.getProducts("model",null,model))[0];
                  const arrivalDate = product.arrivalDate;
                  if (changeDate < arrivalDate) {reject(new DateError()); return;}
                  await this.dao.changeProductQuantity(model,newQuantity,arrivalDate);
    ...
    async sellProduct(model: string, quantity: number, sellingDate: string | null) : Promise<number> { 
        ...
        return new Promise<number>((resolve, reject) => {
            (async () => {
                if(!sellingDate) sellingDate = today;
                else if (sellingDate && sellingDate > today) {reject(new DateError());return;};
                try {
                  const product = (await this.dao.getProducts("model",null,model))[0];
                  const arrivalDate = product.arrivalDate;
                  if (sellingDate < arrivalDate) {reject(new DateError()); return;};
                  if (product.quantity === 0) {reject(new EmptyProductStockError()); return;}; 
                  if (product.quantity < quantity) {reject(new LowProductStockError()); return;};
                  await this.dao.changeProductQuantity(model,-quantity,arrivalDate);
