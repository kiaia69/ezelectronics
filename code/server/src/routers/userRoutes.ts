...
import { UserIsAdminError, UserNotAdminError, UnauthorizedUserError } from "../errors/userError";
...
class UserRoutes {
    ...
    initRoutes() {
         ...
         */
        this.router.post(
            "/", body("username").isString().notEmpty(), body("name").isString().notEmpty(), body("surname").isString().notEmpty(), body("password").isString().notEmpty(),
            body("role").isString().isIn(Object.values(Role)), this.errorHandler.validateRequest,
        ...
        this.router.get(
            ...
            (req: any, res: any, next: any) => {
                if (Utility.isAdmin(req.user) || req.user.username == req.params.username) {
                    return this.controller.getUserByUsername(undefined, req.params.username)
                        .then((user: any /**User */) => res.status(200).json(user))
                        .catch((err) => next(err))

                }
                else {
                    let error = new UserNotAdminError
                    return res.status(error.customCode).json(error.customMessage);
                }
        ...
        this.router.delete(
            ...
            (req: any, res: any, next: any) => {
                ...
                if (req.user.username == req.params.username) {
                    return this.controller.deleteUser(undefined, req.params.username)
                        .then(() => res.status(200).end())
                        .catch((err: any) => {
                            next(err)
                        })
                ...
                if (Utility.isAdmin(req.user)) {
                    ...
                    return this.controller.getUserByUsername(undefined, req.params.username).then(user => {
                        if (Utility.isAdmin(user)) {
                            let error = new UserIsAdminError
                            return res.status(error.customCode).json(error.customMessage);
                        }
                        else {
                            return this.controller.deleteUser(undefined, req.params.username)
                    ...
                    let error = new UnauthorizedUserError
                    return res.status(error.customCode).json(error.customMessage);
                }
            }
        )
        ...
        this.router.delete(
            "/",
            (req: any, res: any, next: any) => this.authService.isAdmin(req, res, next), this.errorHandler.validateRequest,
        ...
        this.router.patch(
            ...
            body("birthdate").isDate({ format: 'YYYY-MM-DD', strictMode: false }),
            this.errorHandler.validateRequest,

            (req: any, res: any, next: any) => {

                //controllo data
                if (new Date(req.body.birthdate).getTime() > new Date().getTime()) {
                    let error = new DateError
                    return res.status(error.customCode).json(error.customMessage);
                }

                if (req.user.username == req.params.username) {
                    return this.controller.updateUserInfo(undefined, req.body.name, req.body.surname, req.body.address, req.body.birthdate, req.params.username)
                        .then((user: any /**User */) => res.status(200).json(user))
                        .catch((err: any) => next(err))
                }
